...
# archivos de salida
archivo=open("no_pub_todos.txt", "w")
archivo1=open("no_act.txt", "w")
archivo2=open("no_pub_desde_2_5.txt", "w")

# cabeceras para cada archivo .txt de salida (Para plotear con google earth)
archivo.write("fecha hora latitud longitud prof mag tipomag analista percibido\n")
archivo1.write("fecha hora latitud longitud prof mag tipomag analista percibido\n")
archivo2.write("fecha hora latitud longitud prof mag tipomag analista percibido\n")
...
for sismo2 in listacsv_2:
    ...
    for sismo1 in listacsv_1:
        ...
        # el delta dia evita que sismos de distinto dia y con horas similares (horas dentro de una ventana de 3 segundos
        ...
        # solo entraran a este if eventos comparados que sean del mismo dia en una ventana de 3 segundos como maximo
        if (delta_dia == 0 or delta_dia_aux== 0) and (delta_seg <= 3 or delta_seg_aux <=3):
            pub=+1
            # el delta maximo en coordenadas es de 1 grado en latitud y longitud
            if delta_lat <= 1 and delta_lon <= 1:
    ...
    if pub==0:
        ...
        #archivo.write(sismo2['fecha_hora']+' '+sismo2['lat']+' '+sismo2['lon']+' '+sismo2['prof']+' '+sismo2['mag']+' '+sismo2['tipo_mag']+' '+sismo2['analista']+' '+sismo1['perc']+"\n")
        ...
        if float(sismo2['mag'])>=2.5:
            ...
            #archivo2.write(sismo2['fecha_hora']+' '+sismo2['lat']+' '+sismo2['lon']+' '+sismo2['prof']+' '+sismo2['mag']+' '+sismo2['tipo_mag']+' '+sismo2['analista']+' '+sismo1['perc']+"\n")
        ...
        if diferencias!=0:
            ...
            #archivo1.write(sismo2['fecha_hora']+' '+sismo2['lat']+' '+sismo2['lon']+' '+sismo2['prof']+' '+sismo2['mag']+' '+sismo2['tipo_mag']+' '+sismo2['analista']+' '+sismo1['perc']+"\n")
...
#print('------------------------------------ fin ------------------------------------')
#print('hora inicio:', fecha_inicio)
#print('hora termino:', fecha_termino)
#print('tiempo proceso:', tiempo_proc)
print('Archivos generados...')
print('total eventos no publicados:', total_eventos, '(',archivo.name,')')
if listanopub:
    print('Eventos no publicados >= a mag. 2.5:', totsobre2_5, '(',archivo2.name,' / ', salida, ')')
print('total eventos no actualizados', total_diferentes, '(',archivo1.name,')')
